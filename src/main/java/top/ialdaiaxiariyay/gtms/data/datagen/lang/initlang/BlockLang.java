...
public class BlockLang {
    ...
    public static void init(UnifiedLanguageProvider provider) {
        ...
        provider.add("block.gtms.spun_time_anchor", "Spun Time Anchor", "纺时之仪");
        provider.add("block.gtms.divergent_dream_rod", "Candle of Alien Dreams", "异梦烛");
        provider.add("block.gtms.time_sand", "Time Sand", "时之沙");
    }
    ...
    public static void LaserSourceHatch(UnifiedLanguageProvider provider) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            for (int amperage : amperages) {
                String key = "block.gtms." + vnLower + "_" + amperage + "a_wireless_laser_source_hatch";
    ...
    public static void LaserTargetHatch(UnifiedLanguageProvider provider) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            String vnf = GTValues.VNF[i];
            for (int amperage : amperages) {
                String key = "block.gtms." + vnLower + "_" + amperage + "a_wireless_laser_target_hatch";
                String formattedAmpere = String.format("%,d", amperage);
