...
public class AutoCollisionBlock extends Block {
    ...
    private VoxelShape computeShapeFromModel() {
        ...
        try (InputStream inputStream = getClass().getResourceAsStream(path)) {
            if (inputStream == null) {
                GTMS.LOGGER.error("Model file not found: {} - using full block collision.", path);
                return Shapes.block();
            }
            ...
            GTMS.LOGGER.error("Failed to parse model file for collision: {}", path);
            GTMS.LOGGER.error(e.getMessage());
            return Shapes.block();
        }
    }
